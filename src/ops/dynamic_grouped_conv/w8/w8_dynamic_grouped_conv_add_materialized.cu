...
void tiled_projection(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    // Ada's static shared-memory ceiling is 48 KiB. The 4096xC schedule reaches 49,664 bytes at
    // C=40 with eight K warps, while four warps retain the same exact contraction in 24,832 bytes.
    constexpr int Warps =
        InputRows == 4096 ? (TileColumns <= 32 ? 8 : 4) : (TileColumns <= 32 ? 8 : 4);
