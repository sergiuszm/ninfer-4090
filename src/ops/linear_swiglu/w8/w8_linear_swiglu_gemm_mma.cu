...
void w8_dflash2_linear_swiglu_mma_r32_c96_k128_launch(const Tensor& x, const Weight& w, Tensor& out,
                                                      cudaStream_t stream) {
    // BM64/C96/K128 needs 50,176 bytes of static shared memory and cannot link for sm_89. BM32
    // preserves one 96-column tile and the K128 contraction while fitting in 37,376 bytes.
    using Schedule = W8RowSplitMmaGemmSchedule<32, 96, 16, 16, 2, 2, 128, 1>;
    launch_route<Schedule>(x, w, out, stream);
}
